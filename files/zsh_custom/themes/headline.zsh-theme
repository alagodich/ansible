...
headline_kubecontext() {
  local ref
  ref=$(kubectl config current-context 2> /dev/null)
  local ret$?
  if [[ $ret == 0 ]]; then
    echo ${ref}
  else
...
headline_precmd() {
  # Information
  local user_str host_str path_str branch_str status_str kube_context_str
  ...
  [[ $HEADLINE_DO_KUBE_CONTEXT == 'true' ]] && kube_context_str=$(headline_kubecontext)
  ...
  if (( ${#kube_context_str} )); then
    _headline_part KUBE_CONTEXT "$HEADLINE_KUBE_CONTEXT_PREFIX [$kube_context_str] " right
  fi
